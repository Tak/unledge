...
class UnledgeShortBus < ShortBus
  # Constructor
  def initialize()
    super
    ...
    @channels = []
    @hooks = []
    hook_command( 'UNLEDGE', XCHAT_PRI_NORM, method( :enable), '')
  ...
  def your_message(words, data)
    ...
    begin
      ...
      if (UNLEDGE_PREFIX_RE.match(words[1]) || !@channels.detect{ |item| item == channel }) then return XCHAT_EAT_NONE; end
